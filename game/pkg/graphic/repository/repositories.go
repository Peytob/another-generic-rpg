...
func (ub UniformBlockRepository) GetById(id uint32) (gresource.UniformBlock, bool) {
	uniformBlock, ok := ub.idIndex[id]
	return uniformBlock, ok
}

func (ub UniformBlockRepository) GetByName(name string) (gresource.UniformBlock, bool) {
	id, ok := ub.nameIndex[name]
	if !ok {
		return gresource.UniformBlock{}, false
	}

	return ub.GetById(id)
}
...
func (ub UniformBlockRepository) Put(uniformBlock gresource.UniformBlock) bool {
	_, exists := ub.GetById(uniformBlock.Id)
	if exists {
		return false
	}
...
func (s ShaderRepository) GetById(id uint32) (gresource.Shader, bool) {
	shader, ok := s.idIndex[id]
	return shader, ok
}

func (s ShaderRepository) GetByName(name string) (gresource.Shader, bool) {
