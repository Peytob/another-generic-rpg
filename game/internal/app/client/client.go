...
func (c *Client) Run(ctx context.Context) error {
	...
	for {
		...
		if c.window.ShouldClose() {
			...
			err = c.runner.Event(ctx, state.StoppedEvent)
			if err != nil {
...
func (c *Client) Shutdown(ctx context.Context) error {
	...
	c.rendering.Terminate(ctx)
	return nil
}

func (c *Client) dumpRunningInfo(ctx context.Context) {
	graphicAPIInfo := c.rendering.Telemetry()
	logger.FromCtx(ctx).LogAttrs(ctx, slog.LevelInfo, "running with rendering",
...
func (c *Client) onWindowSizeChanged(width int, height int) {
	...
	ub, ok := c.rendering.Repositories().Uniform.ByName(rendering.ProjViewUniformBlock)
	if !ok {
